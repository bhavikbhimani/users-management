...
trait FileUploadTrait
{
    ...
    public function saveFile($value, $attribute_name = "image", $destination_path = "", $disk = "")
    {
        ...
        if (Str::startsWith($value, 'data:')) {
            $filename = Str::slug(Config('app.name')) . "-" . md5($value . microtime());
            ...
            return $destination_path . '/' . $filename . $fileext;
            ...
            $filename = Str::slug(Config('app.name')) . "-" . md5($name . microtime());
    ...
    private function saveFileMetaData($file)
    {
        ...
        if (\Schema::hasColumn($this->getTable(), $attrs['file_type'])) {
            ...
            } elseif (Str::startsWith($file, 'http')) {
    ...
    private function base64ToExt($base64_image_string = '')
    {
        ...
        if($mime_split[1] == 'vnd.openxmlformats-officedocument.presentationml.presentation'){
            $mime_split[1] = 'pptx';
        }

        if($mime_split[1] == 'vnd.ms-powerpoint'){
            $mime_split[1] = 'ppt';
        }
